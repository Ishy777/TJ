...
void show_room() // TODO: no arguments passed to this function
{
    ...
    {
        draw_line(
            Walls[Last_Wall_Written][0],
            Walls[Last_Wall_Written][1],
            Walls[Last_Wall_Written][2],
            Walls[Last_Wall_Written][3],
            pen);
    }
...
void draw_mousetrap() {
    ...
    draw_char(round(Mx1), round(My1), Mouse_Trap_IMG);
    draw_char(round(Mx2), round(My2), Mouse_Trap_IMG);
    draw_char(round(Mx3), round(My3), Mouse_Trap_IMG);
    draw_char(round(Mx4), round(My4), Mouse_Trap_IMG);
    draw_char(round(Mx5), round(My5), Mouse_Trap_IMG);
}
void update_mousetrap(){
    ...
    if (round(count_sec/2) == count_sec/2 && Mouse_Traps < 6 ) {
       ...
       setup_mousetrap();}
        ...
        Lives--;
        setup_mousetrap();}
        if (collided(Jx, Jy, Mx1, My1)) {
        Lives--;
        setup_mousetrap();}
        if (collided(Jx, Jy, Mx2, My2)) {
        Lives--;
        setup_mousetrap();}
        if (collided(Jx, Jy, Mx3, My3)) {
        Lives--;
        setup_mousetrap();}
        if (collided(Jx, Jy, Mx4, My4)) {
        Lives--;
         setup_mousetrap();}
        if (collided(Jx, Jy, My5, My5)) {
        Lives--;
        setup_mousetrap();
        }
}
...
void loop()
{
    ...
    if (timer_expired(timer_1s)) count_sec = (count_sec + 1);
    if (count_sec == 60) { 
        count_min++;
        count_sec = 0;
    }
