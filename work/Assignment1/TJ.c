...
void draw_status()
{
    ...
    draw_formatted(1, 2, "Cheese: 1");
    draw_formatted(15, 2, "Traps: 1");
    draw_formatted(30, 2, "Fireworks: 0");
    draw_formatted(45, 2, "Level: 1");
}
...
bool position_is_legal(double new_x, double new_y)
{
    ...
    for (int i = 0; i < Last_Wall_Written; i++)
    {
        ...
        if (new_x >= smaller_x && new_x <= bigger_x &&
            ...
            ((new_x - start_x) * dy) - ((new_y - start_y) * dx) <= 1.0)
        {
            // early exit when illegal coordinates are detected
            return false;
        }
    }
...
void update_jerry(int key_code)
{
    ...
    if (position_is_legal(new_x, new_y))
    {
...
void move_tom()
{
    ...
    if (Jx > Tx && position_is_legal(new_y, new_x + 1))
    {
        new_x += 1;
    }
    else if (position_is_legal(new_y, new_x - 1))
    {
        new_x -= 1;
    }

    if (Jy > Ty && position_is_legal(new_y + 1, new_x))
    {
        new_y += 1;
    }
    else if (position_is_legal(new_y - 1, new_x))
    {
        new_y -= 1;
    }
...
{
    setup_screen();
    FILE *stream = fopen("room02.txt", "r");
    if (stream != NULL)
    {
        setup(stream);
        while (!game_over)
